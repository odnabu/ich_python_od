...
#             subprocess.run(["python", file_path], check=True)       # Можно было ч/з os.system(f'python {args[1]}')
...
if len(sys.argv) != 2:      # т.е. введено ли два аргумента в командную строку после python.
...
if not os.path.isdir(directory_path):
    ...
    sys.exit(1)
# Дальше для обхода директории использую os.walk :
print(f"{'.' * 50}\n"
...
for root, dirs, files in os.walk(directory_path):       # Возвращает три значения для каждой итерации в цикле for.
    ...
    for dir_name in dirs:                               # 2) - Здесь - список поддиректорий в текущей директории.
        # print("------------------ Current", root, dirs, "------------------", sep="-----")   # Для демонстрации.
